...
#GS
#!  Create a GS interative solver function
def GS(A, x_k, x_kP1, b, i):
    ...
    # Sum over j
    for j in range (0, i):
        sum1 += A[i, j] * x_kP1[j] #from the original Jacobi, x_kP1 changed
...
while conv[-1] > convTol:
    ...
    for i in range(0, len(x_k)):
        x_k1[i] = GS(A, x_k, x_k1, b, i)
    conv.append(rel_converg(x_k, x_k1))
    
print "The GS Method took {} iterations and the solution was: \n{}".format(numIter, x_k1)
Error = x_k1 - x_k
print "Error:\n", Error

plt.plot(range(len(conv)), conv,'k',label='GS')
plt.xlabel('Generation',fontsize=12)
plt.ylabel('Relative error [%]',fontsize=12)
plt.title('GS Convergence Plot',fontsize=14)
